...
def update_opposition_service(db: Session, opposition_id: int, update_data: OppositionUpdate):
...
@celery_app.task(name="check_and_send_alerts")
def check_and_send_alerts(db: Session, email: str):
    oppositions = db.query(Opposition).filter(Opposition.alerte_envoyee == False).all()
